  ...
  <div>
    <div class="table-content" :style="{height:tableHeight}">
      <table class="table-result table-gachle" >
        ...
        <tbody>
          <tr v-for="(item, index) in pagerData"  :key="item[id]" @click="onSelectedRow(fromIndexPage+index+1, item)" @dblclick="doubleClickRow(fromIndexPage+index+1, item)" :class="{highlight: rowSelected==fromIndexPage+index+1}">
            ...
            <td v-for="(column, i) in columns" :key="i" :style="column.cssRows" :class="{highlight1: (rowSelected==fromIndexPage+index+1), w40: column.field.toLowerCase()=='stt', 'text-right': typeof item[column.field] === 'number'}">{{item[column.field]}}</td>
...
export default {
    ...
    methods:{
        onChangePage(data){
            ...
            this.fromIndexPage=this.pageIndex*this.pageSize
        },
        ...
        filterChange(field){
            // if(this.filterValues[field].trim()!=''){
            //     this.rowSelected=-1
            // }
        },
        // Filter
