<template>
    <ejs-dialog
        ...
        :visible="false"
        ref="popupProjectedRevenueRegion"
        :showCloseIcon="true"
        width="60%"
        :isModal='true'
        :target="target"
...
export default {
    name:'ProjectedRevenueRegionModal',
    props:{
      isAdd:{
        type:Boolean,
        default:false
    ...
    computed:{
      header(){
        if(this.isAdd){
          return `<span class="icon one-notepad"></span> Tạo mới doanh thu dự kiến địa bàn`
        }else if(!this.isAdd&&!this.isViewDetail){
          return `<span class="icon one-notepad"></span> Điều chỉnh doanh thu dự kiến địa bàn`
        }else{
          return `<span class="icon one-notepad"></span> Doanh thu dự kiến địa bàn`
        }
      },
      formattedRevenue() {
    ...
    methods:{
        showModal() {
            this.$refs["popupProjectedRevenueRegion"].show()
        },
        hideModal() {
            this.$refs["popupProjectedRevenueRegion"].hide()
        },
        handleShowModal(){
            this.clear()
            setTimeout(()=>{
        ...
        async initDuLieu(){
            ...
            if(this.isViewDetail||(!this.isAdd&&!this.isViewDetail)){
                ...
                Object.assign(this.projectedRevenueRegion, this.data)
                this.region_selected=this.projectedRevenueRegion.region_id
                this.center_selected=this.projectedRevenueRegion.center_id
                this.department_selected=this.projectedRevenueRegion.department_id
                this.service_selected=this.projectedRevenueRegion.service_id
                this.hangmuc_selected=this.projectedRevenueRegion.item_id
                this.revenue_type_selected=this.projectedRevenueRegion.revenue_type
                this.field_selected=this.projectedRevenueRegion.field_id
                
                this.input_doanhthu=this.projectedRevenueRegion.revenue?this.projectedRevenueRegion.revenue.toString():''
                this.firstRevenue=this.projectedRevenueRegion.revenue?this.projectedRevenueRegion.revenue.toString():''
                this.tu_thang=this.projectedRevenueRegion.from_period_display?this.projectedRevenueRegion.from_period_display:''
                this.den_thang=this.projectedRevenueRegion.to_period_display?this.projectedRevenueRegion.to_period_display:''


                let result = await API.GetProjectedRevenueRegionInfoById(this, this.data.projected_revenue_region_id)
                if(result){
                    if(result.projectedRevenueRegion){
                        Object.assign(this.projectedRevenueRegion, result.projectedRevenueRegion)
                        this.field_selected=this.projectedRevenueRegion.field_id
                    }
                    if(result.projectedRevenueRegionDetails){
                        let details=result.projectedRevenueRegionDetails.sort((a, b) => a.period - b.period)
            ...
            while (current.isSameOrBefore(to, 'month')) {
                ...
                [1, 2, 3].forEach(type => {
                    ...
                    if (dataSourceMap[key]) {
                        ...
                        result.push({
                            projected_revenue_region_detail_id: 0,
                            revenue: 0,
                            period,
                            period_display,
        ...
        groupRevenueData(data) {
            ...
            data.forEach(item => {
                ...
                group.details.push({
                    ...
                    projected_revenue_region_detail_id:item.projected_revenue_region_detail_id
                });
            });

            return grouped;
        },
        async addProjectedRevenueRegion(){
            //Check
            ...
            let dataRequest={
                ...
                projected_revenue_region_details:this.dataSources.map(x=>{
                    return {
                        projected_revenue_region_detail_id: x.projected_revenue_region_detail_id,
                        revenue: x.revenue,
                        period:x.period,
            ...
            let result=await API.AddProjectedRevenueRegion(this, dataRequest)
            if(result=='1'){
                this.$toast.success('Lưu dữ liệu thành công!')
                this.$emit('successProjectedRevenueRegion')
                this.hideModal()
            }else{
                this.$toast.error(result)
            }
        },
        async updateProjectedRevenueRegion(){
            ...
            let dataRequest={
                projected_revenue_region_id:this.projectedRevenueRegion.projected_revenue_region_id,
                unit_id: unit_id, 
                item_id: this.hangmuc_selected,
                revenue: Number(this.input_doanhthu.trim()), 
                revenue_type:this.revenue_type_selected,
            ...
            let result=await API.UpdateProjectedRevenueRegion(this, dataRequest)
            if(result=='1'){
                this.$toast.success('Lưu dữ liệu thành công!')
                this.$emit('successProjectedRevenueRegion')
                this.hideModal()
        ...
        async onSave(){
            if(this.isAdd){
                this.addProjectedRevenueRegion()
            }else{
                this.updateProjectedRevenueRegion()
            }
        },
        onOpenSubDialog() {
            this.subInputValue = '';
