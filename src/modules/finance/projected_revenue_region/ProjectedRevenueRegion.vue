<template>
  <div class="main-wrapper">
    ...
    <!-- Modal -->
    <ProjectedRevenueRegionModal ref="projectedRevenueRegionModal" 
      :isAdd="isAddNew" 
      :isViewDetail="isViewDetail"
      @successProjectedRevenueRegion="successProjectedRevenueRegion"
      :data="rowSelected"/>
    <CheckImportProjectedRevenueRegionModal ref="checkImportProjectedRevenueRegionModal"
...
import ProjectedRevenueRegionModal from './ProjectedRevenueRegionModal.vue'
import API from './API'
import * as XLSX from 'xlsx-js-style';
import CheckImportProjectedRevenueRegionModal from './CheckImportProjectedRevenueRegionModal.vue'
export default {
  name:'ProjectedRevenueRegion',
  components:{
    ProjectedRevenueRegionModal,
    CheckImportProjectedRevenueRegionModal
  },
  data(){
    return {
      header: {
          title: "Quản lý doanh thu dự kiến địa bàn",
          list: [
          ],
      },
      region_selected:0,
  ...
  methods:{
    ...
    ThemMoi(){
      ...
      this.$refs.projectedRevenueRegionModal.showModal()
    },
    ...
    async ExportExcel(){
      ...
      var result=await API.GetProjectedRevenueRegionExportExcel(this, this.region_selected, this.center_selected, this.department_selected,fromDate, toDate)
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Doanh thu dựa kiến địa bàn');
      XLSX.writeFile(wb, 'doanh_thu_du_kien_dia_ban.xlsx');
    },
    borderAll() {
      return {
    ...
    DownloadImportTemplate(){
      window.open("/static/files/MauFile_Import_Doanh_Thu_Du_Kien_Dia_Ban.xlsx")
    },
    ImportExcel(){
    ...
    async docfile_doanhthu_kehoach_diaban(data){
      let result=await API.CheckImportProjectedRevenueRegion(this, data)
      if(Array.isArray(result)){
        this.dataCheckImports=result
        this.$refs.checkImportProjectedRevenueRegionModal.showModal()
      }else{
    ...
    async deleteRevenue(){
      ...
      let dsId=dsSlected.map(x=>x.projected_revenue_region_id)

      let result = await this.$confirm('Thông báo', `Bạn chắc chắn muốn xóa dữ liệu doanh thu kế hoạch địa bàn đang chọn?`)
      if(result==0) return
      
      let data=await API.DeleteProjectedRevenueRegion(this, JSON.stringify(dsId))
      if(data=='1'){
    ...
    async initDataGrid(){
      let fromDate=Number(moment(this.tu_thang, 'MM/YYYY').format('YYYYMM'))
      let toDate=Number(moment(this.den_thang, 'MM/YYYY').format('YYYYMM'))
      let data=await API.GetProjectedRevenueRegion(this, this.region_selected, this.center_selected, this.department_selected,fromDate, toDate)
      if(data&&data!=''){
