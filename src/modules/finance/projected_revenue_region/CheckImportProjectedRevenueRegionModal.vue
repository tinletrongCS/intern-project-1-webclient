<template>
    <ejs-dialog
        ...
        :visible="false"
        ref="popupCheckImportProjectedRevenueRegion"
        :showCloseIcon="true"
        width="80%"
        :isModal='true'
        :target="target"
...
import API from './API'
export default {
    name:'CheckImportProjectedRevenueRegionModal',
    props:{
      dataCheckImports:{
        type:Array,
        default: () => []
    ...
    methods:{
      showModal() {
          this.$refs["popupCheckImportProjectedRevenueRegion"].show()
      },
      hideModal() {
          this.$refs["popupCheckImportProjectedRevenueRegion"].hide()
      },
      handleShowModal(){
          this.clear()
          setTimeout(()=>{
      ...
      groupRevenueData(data) {
          ...
          data.forEach(item => {
              const key = JSON.stringify({
                  projected_revenue_region_id:item.projected_revenue_region_id,
              ...
              groupedMap.get(key).push({
                  ...
                  projected_revenue_region_detail_id_1:item.projected_revenue_region_detail_id_1,
                  projected_revenue_region_detail_id_2:item.projected_revenue_region_detail_id_2,
                  projected_revenue_region_detail_id_3:item.projected_revenue_region_detail_id_3,
          ...
          const result = Array.from(groupedMap.entries()).map(([key, periods]) => {
              ...
              return {
                  ...
                  projected_revenue_region_details: sorted
              };
          });
      ...
      async onSave(){
        ...
        if(result==0) return
        let dataRequest = this.groupRevenueData(this.dataSources.map(({ projected_revenue_region_id, projected_revenue_region_detail_id, projected_revenue_region_detail_id_1,projected_revenue_region_detail_id_2,projected_revenue_region_detail_id_3, region_name, center_name, department_name, item_revenue_name, period_display, revenue, revenue_type_name, field_name}) => ({
           projected_revenue_region_id, projected_revenue_region_detail_id, projected_revenue_region_detail_id_1,projected_revenue_region_detail_id_2,projected_revenue_region_detail_id_3, region_name, center_name, department_name, item_revenue_name, period_display, revenue, revenue_type_name, field_name
        })));
        let rs=await API.ImportProjectedRevenueRegion(this, dataRequest)
        if(rs=='1'){
