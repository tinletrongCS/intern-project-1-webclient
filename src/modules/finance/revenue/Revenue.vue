<template>
  <div class="main-wrapper">
          ...
          <div class="col-sm-6 col-12">
            <div class="info-row">
              <div class="group-buttons -bottom">
                ...
                <button class="btn btn-second" style="min-width: 100px;" @click.prevent="editActualRevenue()">
                  <span class="icon one-edit"></span>&nbsp;Điều chỉnh
                </button>&nbsp;
                <button class="btn btn-second" style="min-width: 100px;" @click.prevent="deleteActualRevenue()">
    ...
    <!-- Modal -->
    <ActualRevenueModal ref="actualRevenueModal" 
      :isAdd="isAddNew" 
      :isViewDetail="isViewDetail"
      @successActualRevenue="successActualRevenue"
      :data="rowSelected"/>
    <CheckImportActualRevenueModal ref="checkImportActualRevenueModal"
...
import ActualRevenueModal from './ActualRevenueModal.vue'
import API from './API'
import * as XLSX from 'xlsx-js-style';
import CheckImportActualRevenueModal from './CheckImportActualRevenueModal.vue'
export default {
  name:'PlannedRevenue',
  components:{
    ActualRevenueModal,
    CheckImportActualRevenueModal
  },
  data(){
    return {
      header: {
          title: "Quản lý doanh thu thực tế",
          list: [
          ],
      },
      center_selected:0,
      ...
      columns:[
            ...
            {
                fieldName:'revenue_type_display',
                headerText:'Loại doanh thu',
                allowFiltering:true
            },
            {
                fieldName:'revenue',
                headerText:'Doanh thu thực tế (VNĐ)',
                allowFiltering:true,
  ...
  methods:{
    ...
    ThemMoi(){
      ...
      this.$refs.actualRevenueModal.showModal()
    },
    ...
    async ExportExcel(){
      ...
      var result=await API.GetActualRevenueExportExcel(this, this.center_selected, this.deparment_selected, this.group_selected,fromDate, toDate)
      ...
      // 1. Chuẩn bị header
      const wsData = [
        ['Trung tâm', 'Phòng ban', 'Nhóm sản phẩm', 'Tháng', 'Hạng mục', 'Dự án', 'Doanh thu thực tế']
      ];
      
      var data=result.concat([])
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Doanh thu thực tế');
      XLSX.writeFile(wb, 'doanh_thu_thuc_te.xlsx');
    },
    borderAll() {
      return {
    ...
    DownloadImportTemplate(){
      window.open("/static/files/MauFile_Import_Doanh_Thu_Thuc_Te.xlsx")
    },
    ImportExcel(){
    ...
    async commandClicked(cmdName, rowData){
      if(cmdName=='view_detail'){
       ...
       // this.editActualRevenue(rowData)
      }else if(cmdName=='xoa_dulieu'){
       // this.deleteActualRevenue(rowData)
      }
    },
    successImportData(){
      this.initDataGrid()
                    ...
                    if(this.kiemTraDuLieu(columns)){
                      var isError=false
                        for(var i=1;i<data.length;i++){
                          ...
                          var revenue_type=data[i][4]!=undefined?data[i][4].toString().trim():null
                          ...
                          var item_revenue_name=data[i][5]!=undefined?data[i][5].toString().trim():null
                          ...
                          this.danhsach_excel.push({
                              ...
                              project_name:data[i][6]!=undefined?data[i][6].toString().trim():null,
                              revenue:revenue,
                              revenue_type_name:revenue_type,
                          })
                        }
                        if(this.danhsach_excel.length>0&&!isError){
                            this.docfile_doanhthu_thucte(this.danhsach_excel)
                        }
                    }
    ...
    async docfile_doanhthu_thucte(data){
      let result=await API.CheckImportActualRevenue(this, data)
      if(Array.isArray(result)){
        this.dataCheckImports=result
        this.$refs.checkImportActualRevenueModal.showModal()
      }else{
    ...
    //async deleteActualRevenue(rowData){
    async deleteActualRevenue(){
      let dsSlected=this.$refs.gridDs.getSelectedRecords();
      if(dsSlected.length==0){
        this.$toast.error('Vui lòng chọn dữ liệu cần xóa!')
        return
      }
      let dsIdActualRevenue=dsSlected.map(x=>x.actual_revenue_id)

      let result = await this.$confirm('Thông báo', `Bạn chắc chắn muốn xóa dữ liệu doanh thu thực tế đang chọn?`)
      if(result==0) return
      
      let data=await API.DeleteActualRevenue(this, JSON.stringify(dsIdActualRevenue))
      if(data=='1'){
    ...
    async initDataGrid(){
      let fromDate=Number(moment(this.tu_thang, 'MM/YYYY').format('YYYYMM'))
      let toDate=Number(moment(this.den_thang, 'MM/YYYY').format('YYYYMM'))
      let data=await API.GetActualRevenue(this, this.center_selected, this.deparment_selected, this.group_selected,fromDate, toDate)
      if(data&&data!=''){
