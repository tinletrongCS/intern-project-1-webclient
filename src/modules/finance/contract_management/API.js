...
const API_BASE_PATH_REVENUE = 'revenue/contractmanagement';
...
export default {
  ...
  async GetFullContractManagement(parent, contractType) {
    let url = `${API_BASE_PATH_REVENUE}/GetFullContractManagement`;
    if (contractType) {
  ...
  async DeleteContract(parent, ids) {
    try {
      ...
      const response = await axios.delete(`${API_BASE_PATH_REVENUE}/DeleteContractManagement`, {
  ...
  async AddContractManagement(parent, formData) {
    try {
      parent.loading(true);
      const response = await axios.post(`${API_BASE_PATH_REVENUE}/AddContractManagement`, formData, {
  ...
  async UpdateContractManagement(parent, formData) {
    try {
      parent.loading(true);
      const response = await axios.post(`${API_BASE_PATH_REVENUE}/UpdateContractManagement`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      });
      parent.loading(false);
      if (response && response.data && response.data.success) {
  ...
  async GetUsersByDepartmentRevenue(parent) {
    return await this.getDs(parent, `user/Department/GetUsersByDepartmentRevenue`, null, true)
  },
  ...
  async CheckImportContratManagement(parent, data = null) {
    try {
      parent.loading(true)

      const response = await axios.post(
        'revenue/contractmanagement/CheckImportContratManagement',
        data
      )
  ...
  async ImportContractManagementFromExcel(parent, data = null) {
    try {
      parent.loading(true)

      const response = await axios.post(
        'revenue/contractmanagement/ImportContractManagementFromExcel',
        data
      )
  ...
  async UploadFileTaskKanban(parent, fromData, taskId) {
    try {
      parent.loading(true)
      taskId = -1;
      let response = await parent.axios.post('revenue/Upload/UploadFileContractManagement?contractManagementId=' + taskId, fromData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      })
      parent.loading(false)
      if (response && response.data && response.data.success) {
  ...
  async DeleteFileAttachment(parent, file_attachment_id) {
    try {
      parent.loading(true)
      let response = await parent.axios.delete('revenue/Upload/DeleteFileContractManagement', {
        params: { file_attchment_id: file_attachment_id }
      })
      parent.loading(false)
      if (response && response.data && response.data.success) {
        return response.data.data
  ...
  async GetAllUserDepartment(parent) {
    try {
      if (parent.loading) parent.loading(true);

      const response = await axios.get('revenue/contractmanagement/GetAllUserDepartment');
      ...
      if (response?.data?.success) {
        ...
        // Nếu backend trả string JSON -> parse
        if (typeof raw === 'string') {
          try {
            const arr = JSON.parse(raw);
            return Array.isArray(arr) ? arr : [];
          } catch (err) {
            console.error('Invalid JSON in response.data.data:', raw);
            return [];
          }
        }

        // Nếu backend trả sẵn array/object
