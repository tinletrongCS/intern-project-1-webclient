<template>
    <ejs-dialog
        ...
        :visible="false"
        ref="popupCheckImportActualRevenueRegion"
        :showCloseIcon="true"
        width="80%"
        :isModal='true'
        :target="target"
...
import API from './API'
export default {
    name:'CheckImportActualRevenueRegionModal',
    props:{
      dataCheckImports:{
        type:Array,
        default: () => []
    ...
    methods:{
      showModal() {
          this.$refs["popupCheckImportActualRevenueRegion"].show()
      },
      hideModal() {
          this.$refs["popupCheckImportActualRevenueRegion"].hide()
      },
      handleShowModal(){
          this.clear()
          setTimeout(()=>{
      ...
      groupRevenueData(data) {
          ...
          data.forEach(item => {
              const key = JSON.stringify({
                  actual_revenue_region_id:item.actual_revenue_region_id,
              ...
              groupedMap.get(key).push({
                  ...
                  actual_revenue_region_detail_id_1:item.actual_revenue_region_detail_id_1,
                  actual_revenue_region_detail_id_2:item.actual_revenue_region_detail_id_2,
                  actual_revenue_region_detail_id_3:item.actual_revenue_region_detail_id_3,
          ...
          const result = Array.from(groupedMap.entries()).map(([key, periods]) => {
              ...
              return {
                  ...
                  actual_revenue_region_details: sorted
              };
          });
      ...
      async onSave(){
        ...
        if(result==0) return
        let dataRequest = this.groupRevenueData(this.dataSources.map(({ actual_revenue_region_id, actual_revenue_region_detail_id, actual_revenue_region_detail_id_1,actual_revenue_region_detail_id_2,actual_revenue_region_detail_id_3, region_name, center_name, department_name, item_revenue_name, period_display, revenue, revenue_type_name, field_name}) => ({
           actual_revenue_region_id, actual_revenue_region_detail_id, actual_revenue_region_detail_id_1,actual_revenue_region_detail_id_2,actual_revenue_region_detail_id_3, region_name, center_name, department_name, item_revenue_name, period_display, revenue, revenue_type_name, field_name
        })));
        let rs=await API.ImportActualRevenueRegion(this, dataRequest)
        if(rs=='1'){
