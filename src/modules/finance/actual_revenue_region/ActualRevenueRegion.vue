<template>
  <div class="main-wrapper">
    ...
    <!-- Modal -->
    <ActualRevenueRegionModal ref="actualRevenueRegionModal" 
      :isAdd="isAddNew" 
      :isViewDetail="isViewDetail"
      @successActualRevenueRegion="successActualRevenueRegion"
      :data="rowSelected"/>
    <CheckImportActualRevenueRegionModal ref="checkImportActualRevenueRegionModal"
...
import ActualRevenueRegionModal from './ActualRevenueRegionModal.vue'
import API from './API'
import * as XLSX from 'xlsx-js-style';
import CheckImportActualRevenueRegionModal from './CheckImportActualRevenueRegionModal.vue'
export default {
  name:'ActualRevenueRegion',
  components:{
    ActualRevenueRegionModal,
    CheckImportActualRevenueRegionModal
  },
  data(){
    return {
      header: {
          title: "Quản lý doanh thu thực tế địa bàn",
          list: [
          ],
      },
      region_selected:0,
  ...
  methods:{
    ...
    ThemMoi(){
      ...
      this.$refs.actualRevenueRegionModal.showModal()
    },
    ...
    async ExportExcel(){
      ...
      var result=await API.GetActualRevenueRegionExportExcel(this, this.region_selected, this.center_selected, this.department_selected,fromDate, toDate)
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Doanh thu thực tế địa bàn');
      XLSX.writeFile(wb, 'doanh_thu_thuc_te_dia_ban.xlsx');
    },
    borderAll() {
      return {
    ...
    DownloadImportTemplate(){
      window.open("/static/files/MauFile_Import_Doanh_Thu_Thuc_Te_Dia_Ban.xlsx")
    },
    ImportExcel(){
                    ...
                    if(this.kiemTraDuLieu(columns)){
                      var isError=false
                        ...
                        if(this.danhsach_excel.length>0&&!isError){
                            this.docfile_doanhthu_thucte_diaban(this.danhsach_excel)
                        }
                    }
    ...
    async docfile_doanhthu_thucte_diaban(data){
      let result=await API.CheckImportActualRevenueRegion(this, data)
      if(Array.isArray(result)){
        this.dataCheckImports=result
        this.$refs.checkImportActualRevenueRegionModal.showModal()
      }else{
    ...
    async deleteActualRevenue(){
      ...
      let dsId=dsSlected.map(x=>x.actual_revenue_region_id)

      let result = await this.$confirm('Thông báo', `Bạn chắc chắn muốn xóa dữ liệu doanh thu kế hoạch địa bàn đang chọn?`)
      if(result==0) return
      
      let data=await API.DeleteActualRevenueRegion(this, JSON.stringify(dsId))
      if(data=='1'){
    ...
    async initDataGrid(){
      let fromDate=Number(moment(this.tu_thang, 'MM/YYYY').format('YYYYMM'))
      let toDate=Number(moment(this.den_thang, 'MM/YYYY').format('YYYYMM'))
      let data=await API.GetActualRevenueRegion(this, this.region_selected, this.center_selected, this.department_selected,fromDate, toDate)
      if(data&&data!=''){
