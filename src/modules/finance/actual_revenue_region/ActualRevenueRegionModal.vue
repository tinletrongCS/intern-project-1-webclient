...
export default {
    ...
    computed:{
      header(){
        if(this.isAdd){
          return `<span class="icon one-notepad"></span> Tạo mới doanh thu thực tế địa bàn`
        }else if(!this.isAdd&&!this.isViewDetail){
          return `<span class="icon one-notepad"></span> Điều chỉnh doanh thu thực tế địa bàn`
        }else{
          return `<span class="icon one-notepad"></span> Doanh thu thực tế địa bàn`
        }
      },
      formattedRevenue() {
    ...
    methods:{
        showModal() {
            this.$refs["popupActualRevenueRegion"].show()
        },
        hideModal() {
            this.$refs["popupActualRevenueRegion"].hide()
        },
        handleShowModal(){
            this.clear()
            setTimeout(()=>{
        ...
        async initDuLieu(){
            ...
            if(this.isViewDetail||(!this.isAdd&&!this.isViewDetail)){
                ...
                Object.assign(this.actualRevenueRegion, this.data)
                this.region_selected=this.actualRevenueRegion.region_id
                this.center_selected=this.actualRevenueRegion.center_id
                this.department_selected=this.actualRevenueRegion.department_id
                this.service_selected=this.actualRevenueRegion.service_id
                this.hangmuc_selected=this.actualRevenueRegion.item_id
                this.revenue_type_selected=this.actualRevenueRegion.revenue_type
                this.field_selected=this.actualRevenueRegion.field_id
                
                this.input_doanhthu=this.actualRevenueRegion.revenue?this.actualRevenueRegion.revenue.toString():''
                this.tu_thang=this.actualRevenueRegion.from_period_display?this.actualRevenueRegion.from_period_display:''
                this.den_thang=this.actualRevenueRegion.to_period_display?this.actualRevenueRegion.to_period_display:''
                this.firstRevenue=this.actualRevenueRegion.revenue?this.actualRevenueRegion.revenue.toString():''

                let result = await API.GetActualRevenueRegionInfoById(this, this.data.actual_revenue_region_id)
                if(result){
                    if(result.actualRevenueRegion){
                        Object.assign(this.actualRevenueRegion, result.actualRevenueRegion)
                        this.field_selected=this.actualRevenueRegion.field_id
                    }
                    if(result.actualRevenueRegionDetails){
                        let details=result.actualRevenueRegionDetails.sort((a, b) => a.period - b.period)
            ...
            while (current.isSameOrBefore(to, 'month')) {
                ...
                [1, 2, 3].forEach(type => {
                    ...
                    if (dataSourceMap[key]) {
                        ...
                        result.push({
                            actual_revenue_region_detail_id: 0,
                            revenue: 0,
                            period,
                            period_display,
        ...
        groupRevenueData(data) {
            ...
            data.forEach(item => {
                ...
                group.details.push({
                    ...
                    actual_revenue_region_detail_id:item.actual_revenue_region_detail_id
                });
            });

            return grouped;
        },
        async addActualRevenueRegion(){
            //Check
            ...
            let dataRequest={
                ...
                actual_revenue_region_details:this.dataSources.map(x=>{
                    return {
                        actual_revenue_region_detail_id: x.actual_revenue_region_detail_id,
                        revenue: x.revenue,
                        period:x.period,
            ...
            let result=await API.AddActualRevenueRegion(this, dataRequest)
            if(result=='1'){
                this.$toast.success('Lưu dữ liệu thành công!')
                this.$emit('successActualRevenueRegion')
                this.hideModal()
            }else{
                this.$toast.error(result)
            }
        },
        async updateActualRevenueRegion(){
            ...
            let dataRequest={
                actual_revenue_region_id:this.actualRevenueRegion.actual_revenue_region_id,
                unit_id: unit_id, 
                item_id: this.hangmuc_selected,
                revenue: Number(this.input_doanhthu.trim()), 
                revenue_type:this.revenue_type_selected,
            ...
            let result=await API.UpdateActualRevenueRegion(this, dataRequest)
            if(result=='1'){
                this.$toast.success('Lưu dữ liệu thành công!')
                this.$emit('successActualRevenueRegion')
                this.hideModal()
        ...
        async onSave(){
            if(this.isAdd){
                this.addActualRevenueRegion()
            }else{
                this.updateActualRevenueRegion()
            }
        },
        onOpenSubDialog() {
            this.subInputValue = '';
