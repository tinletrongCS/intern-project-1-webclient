...
const INDIVIDUAL_NOTIFICATION_REPORT_API_PATH = 'notifyHub/notification';
...
export default {
  ...
  async GetMyDepartmentTreeOutTTKDGP(parent) {
    const path = `${INDIVIDUAL_WORK_REPORT_API_PATH}/GetMyDepartmentTreeOutTTKDGP`;
  ...
  async GetRemindTasksForMonth(parent, params) {
    const path = `${INDIVIDUAL_NOTIFICATION_REPORT_API_PATH}/GetRemindTasksForMonth`;
    const result = await getDs(parent, path, params, []);
    return result.success ? result.data : [];
  },
  async SendRemindMailSelectedUsers(parent, payload) {
    const path = `${INDIVIDUAL_NOTIFICATION_REPORT_API_PATH}/SendRemindMailSelectedUsers`;

    // payload: { month: "MM/YYYY", userIds: [1,2,3], forceResend: false }
    // successMessage: bạn có thể để null để dùng message từ server
    return await postDs(parent, path, payload, null, true);
  },
