  ...
  methods: {
    ...
    handleShowModal() {
      ...
      setTimeout(() => {
        ...
        this.GetTaskGroupDetailUserMonth()
      }, 200)
    },
    clear() {
      this.dataSources = []
    },
    async GetTaskGroupDetailUserMonth() {
      try {
        if (this.user_id_request == 0) return this.$toast.error('Vui lòng chọn Nhân sự!');
        let response = await API.GetTaskGroupDetailUserMonth(this.axios, {
