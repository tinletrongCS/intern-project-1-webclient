<template>
    <div class="main-wrapper">
            ...
            <div class="row">
                <div class="col-sm-3 col-12">
                    <div class="box-form">
                        ...
                        <select2 class="select2" v-model="userInsert.role_id"
                            :options="listRoles.map(e => ({ id: e.role_id, text: e.role_name }))"></select2>

                        <k-input :textInput='"Username"' :isImportain="true" @modelData='changeDataInputUserName'
                            :dataInput='userInsert.username' />
                        <k-input :textInput='"Password"' :isImportain="true" @modelData='changeDataInputPassword'
                            :dataInput='userInsert.password' />
                        <k-input :textInput='"Telegram Account"' :isImportain="false"
                            @modelData='changeDataInputTelegram' :dataInput='userInsert.telegram_id' />
                        <k-input :textInput='"Telegram Chat ID"' :isImportain="false"
                            @modelData='changeDataInputTelegramChatId' :dataInput='userInsert.telegram_chat_id' />
                        ...
                        <div class="info-row">
                            <div class="group-buttons center -bottom">
                                <button type="Submit" class="btn btn-second" style="min-width: 80px;"
                                    @click="insertUser()" v-show="isInsert" v-if="hasButtonPermission('inserUser')">
                                    <i class="fa fa-plus" aria-hidden="true"></i> Thêm mới
                                </button>
                                <button type="Submit" class="btn btn-second" style="min-width: 80px;" v-show="!isInsert"
                                    @click="updateUser()">
                                    <i class="fa fa-pencil" aria-hidden="true"></i> Update
                                </button>
                                <button type="Submit" class="btn btn-second" style="min-width: 80px;"
                                    :disabled='isDisabled' @click="refreshForm()">
                ...
                <div class="col-sm-9 col-12">
                    <div class="box-form">
                        ...
                        <KDataGrid :columns="colums" :allowFilter="true" :enable-paging-server="false"
                            :dataSource="listUsers" :allowPaging="true" :showFilter="true" :showColumnCheckbox="false"
                            :enabledSelectFirstRow="false" :allowSorting="true" @onRowSelected="selectUser"
                            :commands="commands"
        ...
        <ejs-dialog ref='dialogObj' :allowDragging="true" :enableResize="false" :position="{ X: 'center', Y: 'center' }"
            :visible="false" :showCloseIcon="true" width="40%" :header='"Cập nhật người dùng"' :isModal='true'>
...
<script>
import UserApi from './UserTTKDGPApi.js';
import AccountCpn from './dialog/AccountTTKDGP.vue';
export default {
    components: {
        // KTable,
        // FormInput
    ...
    methods: {
        ...
        async getListUsers() {
            try {
                ...
                const response = await UserApi.getListUsers(this.axios);

                if (response.data?.success) {
                    const raw = response.data.data;

                    // BE trả JSON string => parse
                    // (fallback: nếu BE trả sẵn array/object thì dùng luôn)
                    let arr = [];
                    if (Array.isArray(raw)) {
                        ...
                        // nếu lỡ BE trả 1 object thay vì array
                        arr = Array.isArray(raw) ? raw : [raw];
                    }

                    this.listUsers = (arr || []).map((x) => ({
                        ...x,
                        status_name: x.status == 1 ? "Đang hoạt động" : "Dừng hoạt động",
                    }));

                    this.loading(false);
                } else {
                    this.$toast.error(response.data?.message || "Có lỗi xảy ra");
                    this.loading(false);
                }
            } catch (e) {
                // Nếu JSON.parse lỗi hoặc request lỗi
                this.$toast.error(e?.message || "Có lỗi xảy ra");
                this.loading(false);
            }
        },
         /**
...
<style src="./UserTTKDGP.css" scoped></style>