  ...
  <div class="box-form">
    ...
    <div class="info-row">
      ...
      <div class="value wp100">
        ...
        ></textarea>
        <p class="text-end mb-0" style="font-style: italic">
          {{ input_noidung.length }}/500 kí tự
        </p>
      </div>
    </div>
...
export default {
  name: "AddUpdateRequestModalBase",
  components: {
    AttachFile,
    WorkReportPanel,
  },
  ...
  computed: {
    ...
    mappeddomains() {
      const domainsData =
        this.localDomains.length > 0 ? this.localDomains : this.domains;
      const mapped = domainsData.map((e) => ({
        id: e.domain_id,
        text: e.domain_name,
      }));
      return mapped;
  ...
  methods: {
    /**
     * init() - Được gọi từ sidebar khi mở dialog
     * Dùng để khởi tạo dữ liệu cần thiết
     */
    init() {
      // Reset form để chuẩn bị add mode
    ...
    async copyToClipboard(text) {
      try {
        ...
        if (this.$toast) {
          this.$toast.success("Đã copy mã yêu cầu");
        }
      } catch (error) {
    ...
    async onSaveTask() {
      ...
      try {
        ...
        if (this.editMode && this.currentRequestId) {
          ...
          // Emit success event để sidebar đóng dialog
          this.$emit("success", { status: "ok", data: response });
        } else {
          // Map dữ liệu theo format API
          const mappedData = this.createNewDataRequest(formData);
          const response = await RequestApi.addRequest(mappedData);
          ...
          if (this.$toast) {
            this.$toast.success("Lưu yêu cầu thành công!");
          }

          // Emit success event để sidebar đóng dialog
          this.$emit("success", { status: "ok", data: mappedData });
        }
      } catch (error) {
        if (this.$toast) {
    ...
    onCancel() {
      // Emit cancel event để sidebar đóng dialog
      this.$emit("success", { status: "cancel" });
    },
    ...
    createUpdateDataRequest(requestData) {
      ...
      return {
        ...
        status_id: this.dataLoadFromAPI.status_id,

        planned_duration_time: this.dataLoadFromAPI.planned_duration_time || 1,
