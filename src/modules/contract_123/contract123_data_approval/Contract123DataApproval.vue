<template>
  <div class="main-wrapper">
      ...
      <div class="box-form action-wrapper">
        <div class="row align-items-end">
          ...
          <div class="col-md-6 col-12 text-right action-buttons" style="margin-bottom: 4px !important;">
            <button class="btn btn-primary btn-sm" style="width: 120px !important;" v-if="hasButtonPermission('DataCheckContract123')" @click="onClickDataCheck">
              <span class="icon fa fa-eye"></span> Kiểm tra
            </button>
            <button class="btn btn-success btn-sm" style="width: 120px !important;" v-if="hasButtonPermission('DataApprovalContract123')" @click="onClickDataApproval">
    ...
    /> 
    <Contract123DataReviewConfirmDialog
      ref="reviewDialog"
...
import Contract123DataReviewConfirmDialog from './Contract123DataReviewConfirmDialog.vue'
export default {
  name: 'Contract123DataApproval',
  components: {
    ProjectCreateDialog447,
    Contract447Table,
    Contract123DataReviewConfirmDialog
  ...
  methods: {
    ...
    onClickDataCheck() {
      ...
      this.currentRow = row
      this.$refs.reviewDialog.show(row, 'check')
    },
    ...
    onClickDataApproval() {
      ...
      const row = this.selectedRows[0]
      this.currentRow = row
      this.$refs.reviewDialog.show(row, 'approval')
    },
    async onConfirmDataAction(payload) {
      // payload = { action: 'approve' | 'return' | 'sync', note: '' }

      if (!this.currentRow) {
        this.$toast.error('Không tìm thấy dữ liệu đang xử lý')
        return
      }
      ...
      try {
        if (payload.action === 'approve') {
          // ===== KIỂM TRA DỮ LIỆU =====
          result = await API.ApproveProject447(this, params)

        } else if (payload.action === 'return') {
          // ===== HOÀN TRẢ DỮ LIỆU =====
          if (!payload.note || !payload.note.trim()) {
            this.$toast.error('Vui lòng nhập lý do hoàn trả')
            return
          }

          result = await API.ReturnProject447(this, params)

        } else if (payload.action === 'sync') {
          // ===== THỐNG NHẤT DỮ LIỆU =====
          result = await API.SyncProject447(this, params)

        } else {
          this.$toast.error('Action không hợp lệ')
          return
        }
        ...
        if (result === '1' || result === 1) {
          ...
          this.$toast.error(result || 'Có lỗi xảy ra')
        }
      } catch (e) {
        console.error(e)
        this.$toast.error('Có lỗi xảy ra khi xử lý dữ liệu')
      }
    },
    ...
    async loadDataGrid() {
      try {
        ...
        const data = await API.GetProject447DataApprovalList(this, {
