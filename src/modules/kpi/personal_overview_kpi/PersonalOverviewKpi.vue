<template>
  <div class="main-wrapper">
    ...
    <div class="page-content personal-overview-kpi" style="top: 46px !important; bottom: 46px !important;">
      <div class="box-form">
        <div class="row">
  ...
  methods:{
    ...
    async loadPage(){
      ...
      if(this.unit_list.length>0){
        ...
        this.user_selected=[]
        this.user_list=[]
        this.user_list = await API.GetUsersByDepartment(this, this.department_selected)
    ...
    async loadDailyKpi(isLoading){
      ...
      //GetPersonalOverviewKpiDay
      let period=Number(moment(this.date, 'DD/MM/YYYY').format('YYYYMMDD'))
      let request={
        period:period,
        userId:this.user_selected
      }
      var result=await API.GetPersonalOverviewKpiDay(this, request, isLoading)
      if(result){
    ...
    async loadWeekKpi(isLoading){
      ...
      var result=await API.GetPersonalOverviewKpiWeek(this, request, isLoading)
      if(result){
        result=JSON.parse(result)
    ...
    async loadMonthKpi(isLoading){
      ...
      var result=await API.GetPersonalOverviewKpiMonth(this, request, isLoading)
      if(result){
        result=JSON.parse(result)
...
<style>
.personal-overview-kpi .info-row {
    display: table;
    width: 100%;
    table-layout: fixed; /* Bắt buộc table-cell tuân theo width */
}

.personal-overview-kpi .info-row .key {
    white-space: nowrap;
    overflow: hidden;
...
.personal-overview-kpi .info-row .value {
    display: table-cell;
    vertical-align: middle;
...
.personal-overview-kpi .info-row .value .select2-container {
    width: 100% !important;
}

.personal-overview-kpi.info-row .value .select2-selection {
    width: 100% !important;
    overflow: hidden !important;
}

.personal-overview-kpi .info-row .value .select2-selection__rendered {
 ...
 .personal-overview-kpi .col-button-search{
